...
До першого запиту:
 ...
 Отримано `client_id` та `client_secret` від OLXclient_id та client_secret від OLX
 Зареєстровано всі `redirect_uri`redirect_uri
 Заголовок `Version: 2.0` у всіх запитахVersion: 2.0 у всіх запитах
...
До першої публікації:
 ...
 Ідемпотентність через `external_id` працюєexternal_id працює
До продакшену:

 Черга запитів з retry та backoff на 429
 Refresh-воркер запущено
 Сповіщення `needs_reauth` налаштованоneeds_reauth налаштовано
